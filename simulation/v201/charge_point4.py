...
from ocpp.v201 import ChargePoint as cp4
import os

logging.basicConfig(level=logging.INFO)

class ChargePoint(cp4):
    ...
    async def read_config_and_send_values(self):
        try:
            config_path = os.path.abspath('config_CP_4.json')
...
async def main():
    ...
    config_path = os.path.abspath('config_CP_4.json')  # Ensure this path is correct
    if not os.path.exists(config_path):
        logging.error(f"Configuration file does not exist: {config_path}")
        return

    async with websockets.connect(
            'ws://localhost:9000/CP_4',
            subprotocols=['ocpp2.0.1']
    ) as ws:
        cp4 = ChargePoint('CP_4', ws)

        with open(config_path, 'r') as f:
            config = json.load(f)
            boot_data = config['boot_notification']

        await asyncio.gather(cp4.start(), cp4.send_boot_notification(boot_data))
