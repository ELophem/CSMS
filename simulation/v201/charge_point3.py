...
from ocpp.v201 import ChargePoint as cp3
import os

logging.basicConfig(level=logging.INFO)

class ChargePoint(cp3):
    ...
    async def read_config_and_send_values(self):
        try:
            config_path = os.path.abspath('config_CP_3.json')
...
async def main():
    ...
    config_path = os.path.abspath('config_CP_3.json')  # Ensure this path is correct
    ...
    async with websockets.connect(
        ...
        cp3 = ChargePoint('CP_3', ws)

        with open(config_path, 'r') as f:
            config = json.load(f)
            boot_data = config['boot_notification']

        await asyncio.gather(cp3.start(), cp3.send_boot_notification(boot_data))
